...
class Tab_Fit_2D:
    ...
    def get_eps_alpha( self, name:str ):                                                                # get: eps_alpha
        ...
        New_MNu_true_Nu   = NewEvent_true_SM(row, col, +1, hist, 1297, 2.848, self.dm31, Umix, In_MNu_true_Nu  ).get_event()
        New_MAn_true_Nu   = NewEvent_true_SM(row, col, +1, hist, 1297, 2.848, self.dm31, Umix, In_MAn_true_Nu  ).get_event()
        New_MHE_true_Nu   = NewEvent_true_SM(row, col, +1, hist, 1297, 2.848, self.dm31, Umix, In_MHE_true_Nu  ).get_event()
        New_MNu_true_Anti = NewEvent_true_SM(row, col, -1, hist, 1297, 2.848, self.dm31, Umix, In_MNu_true_Anti).get_event()
        New_MAn_true_Anti = NewEvent_true_SM(row, col, -1, hist, 1297, 2.848, self.dm31, Umix, In_MAn_true_Anti).get_event()
...
if __name__== "__main__":
    ...
    if show == 1:
        Tab_Fit_2D.input_data( 0.307, 0.02195, 0.561, 0.9833*np.pi, 2.534*1e-3, 0*np.pi, 40 ).get_eps_alpha('DUNE_Near9000_NSI_fit2D_WithBG_Eps_alpha')
        Tab_Fit_2D.input_data( 0.307, 0.02195, 0.561, 0.9833*np.pi, 2.534*1e-3, 1*np.pi, 40 ).get_eps_alpha('DUNE_Near9000_NSI_fit2D_WithBG_PiEps_alpha')
    ...
    if show == 3:
        ...
        New_MNu_true_Nu   = NewEvent_true_SM(row, col, +1, hist, 1297, 2.848, 2.534*1e-3, Umix, In_MNu_true_Nu  ).get_event()

        #new_Near_MNu_Nu  = NewEvent_Near_reco_NSI.input_data( row, col, +1, hist, 561, 0, self.dm31,Umix, New_MAn_true_Anti )
        #Near_reco_MAn_Anti = new_Near_MAn_Anti.get_event( Norm5x5_MAn_Anti, 0, 0 )
    ...
    if show == 4:
        ...
        New_Near_pre_MNu_Nu   = NewEvent_Near_reco_NSI.input_data(row, col, +1, hist, 0.574, dens, m_31, Umix, New_MNu_true_Nu  ).get_event(Norm5x5_MNu_Nu, 0, 0 )
        ...
        New_Near_pre_MAn_Nu   = NewEvent_Near_reco_NSI.input_data(row, col, +1, hist, 0.574, dens, m_31, Umix, New_MAn_true_Nu  ).get_event(Norm5x5_MAn_Nu, 0, 0  )
        ...
        New_Near_pre_MHE_Nu   = NewEvent_Near_reco_NSI.input_data(row, col, +1, hist, 0.574, dens, m_31, Umix, New_MHE_true_Nu  ).get_event(Norm5x5_MHE_Nu, 0, 0  )
        ...
        New_Near_pre_MNu_Anti = NewEvent_Near_reco_NSI.input_data(row, col, -1, hist, 0.574, dens, m_31, Umix, New_MNu_true_Anti).get_event(Norm5x5_MNu_Anti, 0, 0)
        ...
        New_Near_pre_MAn_Anti = NewEvent_Near_reco_NSI.input_data(row, col, -1, hist, 0.574, dens, m_31, Umix, New_MAn_true_Anti).get_event(Norm5x5_MAn_Anti, 0, 0)
