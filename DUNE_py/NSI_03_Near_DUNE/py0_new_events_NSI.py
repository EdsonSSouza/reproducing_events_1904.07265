...
if __name__ == "__main__":
    ...
    if show == -1:
        PMNS = Matrix_Osc.input_data( 0.307, 0.02203, 0.572, (190/180)*np.pi )
        mass = 2.511*1e-3
        Near_ratio_NSI = Prob_Near_ratio_NSI( 1, 2, +1, 2.1, 0.574, 0, mass, PMNS ).get_osc(1*1e-3, 0*np.pi)
        print( f"{Near_ratio_NSI}\n" )    

    if show == 0:
        PMNS = Matrix_Osc.input_data( 0.307, 0.02195, 0.561, (177/180)*np.pi )
        ...
        matrix_PMNS = Matrix_Osc.input_data( 0.307, 0.02195, 0.561, (177/180)*np.pi )
        mass =  2.534*1e-3
