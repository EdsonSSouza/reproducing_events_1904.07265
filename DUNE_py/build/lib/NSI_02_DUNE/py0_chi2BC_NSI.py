...
class Chi2_build:                                                                                      # Chi^2_i: i_th element of the chi^2_BC ( Baker and Cousins )
    def __init__(self, data:float, model:float) -> None:
...
class Chi2BC_NSI:                                                                                      # For 3.5 + 3.5 years: Mode_Nu and Mode_An 
    """ 
    For 3.5 + 3.5 years: Mode_Neutrino and Mode_Antineutrino

        class Chi2BC_NSI has 5 different chi2_BC:
    ...
    def __init__( self, Calc_MNu_reco_Nu:np.ndarray, Calc_MAn_reco_Nu:np.ndarray, Calc_MNu_reco_Anti:np.ndarray, Calc_MAn_reco_Anti:np.ndarray, BG_On_or_Off:int=1 ) -> None:
    ...
    @classmethod
    def input_data( cls, Calc_MNu_reco_Nu:np.ndarray, Calc_MAn_reco_Nu:np.ndarray, Calc_MNu_reco_Anti:np.ndarray, Calc_MAn_reco_Anti:np.ndarray, BG_On_or_Off:int=1 ):
        return cls( Calc_MNu_reco_Nu, Calc_MAn_reco_Nu, Calc_MNu_reco_Anti, Calc_MAn_reco_Anti, BG_On_or_Off )

    def get_MNu_Nu( self, alpha:float, delta_mut:float ) -> float:                                     # get_MNu_Nu
    ...
    def get_MAn_Nu( self, alpha:float, delta_mut:float ) -> float:                                     # get_MAn_Nu
        """ Antineutrino Mode: Neutrino """
        err_sys   = (4*alpha)**2                                                                       # Systematic error (25%): alpha/0.25 = 4*alpha
        numb_loop = len( In_pre_MAn_Nu )
        if len( self.cal_MAn_Nu ) != numb_loop:
            raise Exception( " Number different from the data entered ! " )
        Ndat_Nu = New_pre_MAn_Nu    
    ...
    def get_MNu_Anti( self, alpha:float, delta_mut:float ) -> float:                                   # get_MNu_Anti
        """ Neutrino Mode: Antineutrino """
        err_sys   = (4*alpha)**2                                                                       # Systematic error (25%): alpha/0.25 = 4*alpha
        numb_loop = len( In_pre_MNu_Anti )
        if len( self.cal_MNu_Anti ) != numb_loop:
            raise Exception( " Number different from the data entered ! " )
        Ndat_Anti = New_pre_MNu_Anti
    ...
    def get_MAn_Anti( self, alpha:float, delta_mut:float ) -> float:                                   # get_MAn_Anti
        """ Antineutrino Mode: Antineutrino """
        err_sys   = (4*alpha)**2                                                                       # Systematic error (25%): alpha/0.25 = 4*alpha
        numb_loop = len( In_pre_MAn_Anti )
        if len( self.cal_MAn_Anti ) != numb_loop:
            raise Exception( " Number different from the data entered ! " )
        Ndat_Anti = New_pre_MAn_Anti
    ...
    def get_all( self, alpha:float, delta_mut:float ) -> float:                                        # get_all: Modes and Particles
...
class Chi2BC_331_NSI:                                                                                  # For 3 + 3 + 1 years: Mode_Nu and Mode_An
    """ For 3 + 3 + 1 years: Mode_Neutrino and Mode_Antineutrino

        class Chi2BC_331_NSI has 6 different chi2_BC:
    ...
    def __init__( self, Calc_MNu_reco_Nu:np.ndarray, Calc_MAn_reco_Nu:np.ndarray, 
                        Calc_MHE_reco_Nu:np.ndarray, Calc_MNu_reco_Anti:np.ndarray, 
                        Calc_MAn_reco_Anti:np.ndarray, BG_On_or_Off:int=1 ) -> None:
        ...
        if BG_On_or_Off == 1 or BG_On_or_Off == 0:                   
            ...
            Exception( " If there is BG select 1 (On) or 0 (Off) if there is not ! " )
    @classmethod
    def input_data( cls, Calc_MNu_reco_Nu:np.ndarray, Calc_MAn_reco_Nu:np.ndarray, 
                         Calc_MHE_reco_Nu:np.ndarray, Calc_MNu_reco_Anti:np.ndarray, 
                         Calc_MAn_reco_Anti:np.ndarray, BG_On_or_Off:int=1 ):
    ...
    def get_MHE_Nu( self, alpha:float, delta_mut:float ) -> float:                                     # get_MHE_Nu
...
if __name__=="__main__":
    ...
    if show == 1:
        ...
        chi2_MNu_Nu = Chi2BC_NSI.input_data( In_pre_MNu_Nu, In_pre_MAn_Nu, In_pre_MNu_Anti, In_pre_MAn_Anti ).get_MNu_Nu(0.0, 1*1e-3)
        chi2_all    = Chi2BC_NSI.input_data( In_pre_MNu_Nu, In_pre_MAn_Nu, In_pre_MNu_Anti, In_pre_MAn_Anti ).get_all(0.0, 1*1e-3)
        print(chi2_MNu_Nu)
        print(chi2_all)
        
    elif show == 3:
        chi2_MNu_Nu = Chi2BC_NSI.input_data( [.1*i for i in In_pre_MNu_Nu], In_pre_MAn_Nu, In_pre_MNu_Anti, In_pre_MAn_Anti   ).get_MNu_Nu(0.1, 1*1e-3)
        print(f"alpha = {0.1}, fac={0.1}:\t{chi2_MNu_Nu}")
        
        chi2_MAn_Nu = Chi2BC_NSI.input_data( In_pre_MNu_Nu, [.1*i for i in In_pre_MAn_Nu], In_pre_MNu_Anti, In_pre_MAn_Anti   ).get_MAn_Nu(0.1, 1*1e-3)
        print(f"alpha = {0.1}, fac={0.1}:\t{chi2_MAn_Nu}")
        
        chi2_MNu_Anti = Chi2BC_NSI.input_data( In_pre_MNu_Nu, In_pre_MAn_Nu, [.1*i for i in In_pre_MNu_Anti], In_pre_MAn_Anti ).get_MNu_Anti(0.1, 1*1e-3)
        print(f"alpha = {0.1}, fac={0.1}:\t{chi2_MNu_Anti}")
        
        chi2_MAn_Anti = Chi2BC_NSI.input_data( In_pre_MNu_Nu, In_pre_MAn_Nu, In_pre_MNu_Anti, [.1*i for i in In_pre_MAn_Anti] ).get_MAn_Anti(0.1, 1*1e-3)
        print(f"alpha = {0.1}, fac={0.1}:\t{chi2_MAn_Anti}")
        
        print( chi2_MNu_Nu + chi2_MAn_Nu + chi2_MNu_Anti + chi2_MAn_Anti )
        chi2_all = Chi2BC_NSI.input_data( [.1*i for i in In_pre_MNu_Nu], [.1*i for i in In_pre_MAn_Nu], [.1*i for i in In_pre_MNu_Anti], \
                                          [.1*i for i in In_pre_MAn_Anti] ).get_all(0.1, 1*1e-3)
        print(f"alpha = {0.1}, fac={0.1}:\t{chi2_all}")

    elif show == 4:
        chi2_MNu_Nu = Chi2BC_331_NSI.input_data( In_pre_MNu_Nu, In_pre_MAn_Nu, In_pre_MHE_Nu, In_pre_MNu_Anti, In_pre_MAn_Anti ).get_MNu_Nu(0.0, 1*1e-3)
        chi2_all    = Chi2BC_331_NSI.input_data( In_pre_MNu_Nu, In_pre_MAn_Nu, In_pre_MHE_Nu, In_pre_MNu_Anti, In_pre_MAn_Anti ).get_all(0.0, 1*1e-3)
        print(chi2_MNu_Nu)
        print(chi2_all)
    
    elif show == 5:
        chi2_MNu_Nu = Chi2BC_331_NSI.input_data( [.1*i for i in In_pre_MNu_Nu], In_pre_MAn_Nu, In_pre_MHE_Nu, In_pre_MNu_Anti, In_pre_MAn_Anti   ).get_MNu_Nu(0.1, 1*1e-3)
        print(f"alpha = {0.1}, fac={0.1}:\t{chi2_MNu_Nu}")
        
        chi2_MAn_Nu = Chi2BC_331_NSI.input_data( In_pre_MNu_Nu, [.1*i for i in In_pre_MAn_Nu], In_pre_MHE_Nu, In_pre_MNu_Anti, In_pre_MAn_Anti   ).get_MAn_Nu(0.1, 1*1e-3)
        print(f"alpha = {0.1}, fac={0.1}:\t{chi2_MAn_Nu}")
        
        chi2_MHE_Nu = Chi2BC_331_NSI.input_data( In_pre_MNu_Nu, In_pre_MAn_Nu, [.1*i for i in In_pre_MHE_Nu], In_pre_MNu_Anti, In_pre_MAn_Anti   ).get_MHE_Nu(0.1, 1*1e-3)
        print(f"alpha = {0.1}, fac={0.1}:\t{chi2_MHE_Nu}")

        chi2_MNu_Anti = Chi2BC_331_NSI.input_data( In_pre_MNu_Nu, In_pre_MAn_Nu, In_pre_MHE_Nu, [.1*i for i in In_pre_MNu_Anti], In_pre_MAn_Anti ).get_MNu_Anti(0.1, 1*1e-3)
        print(f"alpha = {0.1}, fac={0.1}:\t{chi2_MNu_Anti}")
        
        chi2_MAn_Anti = Chi2BC_331_NSI.input_data( In_pre_MNu_Nu, In_pre_MAn_Nu, In_pre_MHE_Nu, In_pre_MNu_Anti, [.1*i for i in In_pre_MAn_Anti] ).get_MAn_Anti(0.1, 1*1e-3)
        print(f"alpha = {0.1}, fac={0.1}:\t{chi2_MAn_Anti}")
        
        print( chi2_MNu_Nu + chi2_MAn_Nu + chi2_MHE_Nu + chi2_MNu_Anti + chi2_MAn_Anti )
        chi2_all = Chi2BC_331_NSI.input_data( [.1*i for i in In_pre_MNu_Nu], [.1*i for i in In_pre_MAn_Nu], [.1*i for i in In_pre_MHE_Nu], \
                                              [.1*i for i in In_pre_MNu_Anti], [.1*i for i in In_pre_MAn_Anti] ).get_all(0.1, 1*1e-3)
        print(f"alpha = {0.1}, fac={0.1}:\t{chi2_all}")
