...
class NewEvent_reco_SM:
    ...
    def get_event( self, Norm_type:float ) -> np.ndarray :
...
if __name__ == "__main__":
    ...
    if show == 0:
        ...
        for i in range(3):
            for j in range(3):
                ratio_SM[i][j] = Prob_ratio_SM( i, j, +1, 3.7, 1297, 2.84, mass, PMNS ).get_osc()
        print( f"{ratio_SM}\n" )    
    
    elif show == 1:
        PMNS_BF = Matrix_Osc.input_data( 0.307, 0.02195, 0.561, (177/180)*np.pi )
        mass_BF = 2.534*1e-3
        ratio_SM = [ Prob_ratio_SM( 1, 2, +1, en, 1297, 2.84, mass_BF, PMNS_BF ).get_osc()  for en in energy ]
        print( f"{[round(vet,3) for vet in ratio_SM]}\n" )

        In_event = In_MNu_true_Nu
        norm_fac = Norm5x5_MNu_Nu
        new_event = NewEvent_reco_SM.input_data( 1, 2, +1, hist, L, dens, mass_BF, PMNS_BF, In_event  ).get_event( norm_fac )
        
        In_pre   = In_pre_MNu_Nu
        ...
        mass =  2.29*1e-3
        ratio_SM = Prob_ratio_SM( 1, 2, +1, 3.51, 1297, 2.84, mass, matrix_PMNS ).get_osc()
        print( f"{ratio_SM}\n" )
        ...
        matrix_PMNS = Matrix_Osc.input_data( 0.35, 0.041, 0.5, 0.5*np.pi )
        mass =  2.29*1e-3
        ratio_SM = [ round(Prob_ratio_SM( 1, 2, +1, en, 1297, 2.84, mass, matrix_PMNS ).get_osc(), 2)  for en in energy ]
        print( f"{ratio_SM}\n" )
