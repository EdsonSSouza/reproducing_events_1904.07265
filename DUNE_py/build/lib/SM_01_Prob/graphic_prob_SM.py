...
PMNS = Matrix_Osc.input_data( 0.307, 0.02195, 0.561, 0.9833*np.pi )
mass = Mass_order.input_data( 7.49*1e-5, 2.534*1e-3 )
prob_mut = Probability_SM.input_data( +1, 0.01, 1300, PMNS, mass, 2.848 ).get_osc_SM()[1][2]

def ProbInf_mut( energy ):
    PMNS_i = Matrix_Osc.input_data( 0.307, 0.00001, 0.5, 0.0*np.pi )
    mass_i = Mass_order.input_data( 7.49*1e-5, 2.534*1e-3 )
    return [ Probability_SM.input_data( +1, en, 1300, PMNS_i, mass_i, 2.848 ).get_osc_SM()[1][2]    for en in energy ]

def Prob_BF( energy ):
    PMNS_i = Matrix_Osc.input_data( 0.307, 0.02195, 0.561, 0.0*np.pi )
    mass_i = Mass_order.input_data( 7.49*1e-5, 2.534*1e-3 ) 
    return [ Probability_SM.input_data( +1, en, 1300, PMNS_i, mass_i, 2.848 ).get_osc_SM()[1][2]    for en in energy ]

def ProbSup_mut( energy ):
    PMNS_i = Matrix_Osc.input_data( 0.307, 0.02195, 0.154738, 0.0*np.pi )
    mass_i = Mass_order.input_data( 7.49*1e-5, 2.534*1e-3 )
    return [ Probability_SM.input_data( +1, en, 1300, PMNS_i, mass_i, 2.848 ).get_osc_SM()[1][2]    for en in energy ]
