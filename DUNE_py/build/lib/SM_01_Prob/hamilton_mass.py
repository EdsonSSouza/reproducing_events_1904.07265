...
class Hamilton_matrix:
    """ 
        Hamiltonian for the Standard Model (SM)
    ...
    def __init__( self, sign_cp:int, energy:float, matter_V:float, instancia_U_PMNS, instancia_M_order ) -> None:
    ...
    @classmethod
    def input_data( cls, sign_cp:int, energy:float, matter_V:float, instancia_U_PMNS, instancia_M_order ):
        return cls( sign_cp, energy, matter_V, instancia_U_PMNS, instancia_M_order )
      
    def get_base_mass(self) -> np.ndarray:                                                                # Definite of the Hamiltoniana: mass base
        Nu_Flavor = 3                                                                                     # Number of flavor neutrinos
        energy_inv = 1/self.energy

        Hf_bm  = np.zeros( (Nu_Flavor, Nu_Flavor), dtype=complex )
        M_prod = np.zeros( (Nu_Flavor, Nu_Flavor), dtype=complex )                                        # M_prod: product matrix
