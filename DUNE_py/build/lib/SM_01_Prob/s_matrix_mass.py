...
class S_matrix:
    ...
    def __init__(self, sign_cp:int, energy:float, distance_L:float, matter_V:float, instancia_U_PMNS, instancia_M_order) -> None:
    ...
    @classmethod
    def input_data( cls, sign_cp:int, energy:float, distance_L:float, matter_V:float, instancia_U_PMNS, instancia_M_order):
        return cls( sign_cp, energy, distance_L, matter_V, instancia_U_PMNS, instancia_M_order )
    
    def get_S_bm(self) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    Sf_bm = S_matrix.input_data( +1, 0.1, 1297*5.0677302143*1e9, 2.848*0.5*7.63247*1e-14, matrix_PMNS, matrix_mass )
    print(f"\n{Sf_bm.get_S_bm()}\n") 
