...
class Gaussian_interp2D:
    _instance = None
    def __init__(self, energy, factor_keep:float, factor_sqrt:float, factor_linear:float, factor_mean:float, normalized:int) -> None:
        self.en = energy    
    ...
    @classmethod
    def input_data( cls, energy:np.ndarray ):
        setup_data    = cls( energy, None, None, None, None, None )
        cls._instance = setup_data
        return cls._instance
    @classmethod
    def input_change( cls, factor_keep:float, factor_sqrt:float, factor_linear:float, factor_mean:float, normalized:int ):
    ...
    def get_function2D(self) -> np.ndarray:                                                                         # Function that returns "interpolate" in 2D of Migrate Matrix
...
class Mapping_matrix:
    _instance = None
    def __init__( self, energy:np.ndarray, factor_keep:float, factor_sqrt:float, factor_linear:float, factor_mean:float ):
        self.en = energy
    ...
    def input_data(cls, energy:np.ndarray):
        ...
        return cls._instance
    @classmethod
    def input_change(cls, factor_keep:float, factor_sqrt:float, factor_linear:float, factor_mean:float):
        if cls._instance is None:
    ...
    def get_mapping_mean(self) -> np.ndarray:                                                           # Function that maps En_reco(row) <-> En_true(column) "to the bin average"
