...
print( f"{mass.get_ordering()}\n" )

Upmns = Matrix_Osc.input_data( 0.310, 0.02240, 0.582, 1.204225*np.pi )
print( f"{Upmns.get_U()}\n" )

ham = Hamilton_matrix.input_data( +1, 3.01, 2.848*0.5*7.63247*1e-14*1e9, Upmns, mass )
print( f"{ham.get_base_mass()}\n" )

S_mat = S_matrix.input_data( +1, 3.01, 5.0677302143*1e9*1300, 2.848*0.5*7.63247*1e-14, Upmns, mass )
print( f"{S_mat.get_S_bm()}\n" )

prob_SM = Probability_SM.input_data( +1, 3.01, 1300, Upmns, mass, 2.848 )
print( f"{prob_SM.get_osc_SM()}\n" )

prob2_SM = Probability_SM.input_data( -1, 3.01, 1300, Upmns, mass, 2.848 )
print( f"{prob2_SM.get_osc_SM()}\n" )
