...
# Rules for converting bins_true into bins_reco using mapping
class Rule_smear:
    _instance = None
    ...
    @classmethod
    def change_parameter( cls, factor_linear, factor_mean, normalized ):
    ...
    def get_middle(self):                                                                                                       # Midpoint
        ...
        mapping_mid = Gaussian_interp2D.input_data(energy).change_parameter(0, 0, self.fac_linear, self.fac_mean, self.norm).get_function2D()   # Mapping Gaussian_2D
    ...
    def get_5to5(self):                                                                                                         # 5x5 points
        energy  = np.linspace(0, 20, 401)
        map_gau = Gaussian_interp2D.input_data(energy).change_parameter(0, 0, self.fac_linear, self.fac_mean, self.norm).get_function2D()    # Mapping Gaussian_2D
    ...
    def get_5to5_pre(self):                                                                                                     # Pre-calculation: 5x5 points
        if self.fac_linear != 0.256 or self.fac_mean != 0.436 or self.norm != 1:
            raise Exception(" This object is only valid for .change_parameter(0.256, 0.436, 1) ! ") 
...
if __name__ == "__main__":
    ...
    if show == 1:
        #event_minus_reco = Rule_smear.input_data( hist, In_MNu_true_Nu  ).change_parameter(0.25, 0.43, 1).get_middle()
        #event_plus_reco  = Rule_smear.input_data( hist, In_MNu_true_Anti).change_parameter(0.25, 0.43, 1).get_middle()
    ...
    if show == 2:
        event_minus_reco = Rule_smear.input_data( hist, In_MNu_true_Nu  ).change_parameter(0.256,0.436,1).get_5to5_pre()
        ...
        energy  = np.linspace(0, 20, 401)
        map_gau = Gaussian_interp2D.input_data(energy).change_parameter(0, 0, 0.256, 0.436, 1).get_function2D()
