...
class Graph_fill:
    ...
    @classmethod
    def input_data(cls, type_mode, histogram, In_BG, Calc_reco_Nu, Calc_reco_Anti):
        setup_hist    = cls( type_mode, histogram, In_BG, Calc_reco_Nu, Calc_reco_Anti, None )
        cls._instance = setup_hist 
        return cls._instance
    @classmethod
    def input_change(cls, In_comparison):
...
class Graph_fill_HE:
    ...
    @classmethod
    def input_data(cls, histogram, In_BG, Calc_reco_Nu):
...
if __name__ == "__main__":
    ...
    if show == 2:
        # Neutrino Mode (Calculate):                                                                            # Neutrino (Calc.): Reconstruction
        cal_MNu_reco_Nu   = Rule_smear.input_data( hist, In_MNu_true_Nu  ).change_parameter(0.2, 0.3, 1).get_5to5()       # Mode_Nu : Events/bin Neutrino
        cal_MNu_reco_Anti = Rule_smear.input_data( hist, In_MNu_true_Anti).change_parameter(0.2, 0.3, 1).get_5to5()       # Mode_Nu : Events/bin Antineutrino
        ...
        # Antineutrino Mode (Calculate):                                                                        # Antineutrino (Calc.): Reconstruction
        cal_MAn_reco_Nu   = Rule_smear.input_data( hist, In_MAn_true_Nu  ).change_parameter(0.2, 0.3, 1).get_5to5()       # Mode_An : Events/bin Neutrino
        cal_MAn_reco_Anti = Rule_smear.input_data( hist, In_MAn_true_Anti).change_parameter(0.2, 0.3, 1).get_5to5()       # Mode_An : Events/bin Antineutrino
        plt_Anti = Graph_fill_Cp.input_data( -1, hist, In_MAn_BG, cal_MAn_reco_Nu, cal_MAn_reco_Anti)
        plt_Anti.get_plot_bar()

        # High Energy Mode (Calculate):                                                                         # High Energy (Calc.): Reconstruction
        cal_MHE_reco_Nu = Rule_smear.input_data( hist, In_MHE_true_Nu ).change_parameter(0.2, 0.3, 1).get_5to5()          # Mode_HE : Events/bin Neutrino        
