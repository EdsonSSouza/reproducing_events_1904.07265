...
class Gaussian_interp2D:
    ...
    @classmethod
    def change_parameter( cls, factor_keep, factor_sqrt, factor_linear, factor_mean, normalized ):
...
class Mapping_matrix:
    ...
    def input_data(cls, energy):
        ...
        return cls._instance
    @classmethod
    def change_parameter(cls, factor_keep, factor_sqrt, factor_linear, factor_mean):
        if cls._instance is None:
...
if __name__ == "__main__":
    ...
    if show == 1:
        ...
        new_directory = '/home/edson/Projeto_doutorado/Experimentos/Beam_Tau/Python/Init_00_tau'
        os.chdir( new_directory )

        energy = np.linspace(0, 20, 401)
        
        # Normalized
        gau_interp = Gaussian_interp2D.input_data(energy).change_parameter(0, 0, 0.256, 0.436, 1).get_function2D()
        Z_new = gau_interp(energy, energy)
        
        # Non-Normalized
        Non_interp = Gaussian_interp2D.input_data(energy).change_parameter(0, 0, 0.256, 0.436, 0).get_function2D()
