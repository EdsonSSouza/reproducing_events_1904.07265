        ...
        with open(path_file, 'w') as save_file:
            ...
            for i in range( interval+1 ):
                for j in range( interval+1 ):
                    ...
                    Upmns = Matrix_Osc.input_data( 0.310, s2_13_i, s2_23_j, -0.79444*np.pi )
                    ...
                    save_file.write( "{:<12} {:<12} {:<12}\n".format( round(s2_13_i, 7), round(s2_23_j, 7), round(chi2_BC, 10) ) )
...
class Tab_Fit_2D:
    ...
    def get_s13_s23( self ):
        ...
        dir = '/home/edson/Projeto_doutorado/Experimentos/Beam_Tau/Python/02_SM_DUNE/my_data/'
        # Nome do arquivo para salvar
        name_file = "DUNE_fit_SM_s13_s23.dat"
        #name_file = "Neut_fit_SM_s13_s23.dat"
        # Caminho completo do arquivo
        path_file = os.path.join(dir, name_file)
        ...
        with open(path_file, 'w') as save_file:
            ...
            for i in range( self.interv + 1 ):
                for j in range( self.interv + 1 ):
                    ...
                    Upmns = Matrix_Osc.input_data( self.s2_12, s2_13_i, s2_23_j, self.del_cp )

                    EvNew_Nu_TauMinus_reco = NewEvent_Smearing_SM.input_data( row, col, +1, hist, L, dens, 2.525*1e-3, Upmns, In_minus_Nu_true ).get_reco()
                    EvNew_Nu_TauPlus_reco  = NewEvent_Smearing_SM.input_data( row, col, +1, hist, L, dens, 2.525*1e-3, Upmns, In_plus_Nu_true  ).get_reco()
                    ...
                    EvNew_Anti_TauMinus_reco = NewEvent_Smearing_SM.input_data( row, col, -1, hist, L, dens, 2.525*1e-3, Upmns, In_minus_AntiNu_true ).get_reco()
                    EvNew_Anti_TauPlus_reco  = NewEvent_Smearing_SM.input_data( row, col, -1, hist, L, dens, 2.525*1e-3, Upmns, In_plus_AntiNu_true  ).get_reco()
                    ...
                    EvNew_HE_TauMinus_reco = NewEvent_Smearing_SM.input_data( row, col, +1, hist, L, dens, 2.525*1e-3, Upmns, In_minus_HE_true ).get_reco()
...
if __name__== "__main__":
    ...
    if show == 1:
        ...
        Tab_Fit_2D.input_data( 0.310, 0.02240, 0.582, -0.79444*np.pi, 20 ).get_s13_s23()
